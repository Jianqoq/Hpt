...
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::bn_remain_oc_kernel_dispatch;
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::conv2d_full_oc_bias_kernel_dispatch;
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::conv2d_full_oc_kernel_dispatch;
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::remain_oc_kernel_dispatch;
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::Params;
use crate::backends::cpu::kernels::conv2d::micro_kernels::batch_norm_conv::PartialParams;
    ...
    A::Output: AllocatorOutputRetrive,
{
    ...
    let bias_full_oc_kernel =
        conv2d_full_oc_bias_kernel_dispatch::<T>([kh, kw], &mut oc_nvec, &mut ow_block);
    let bias_one_oc_kernel =
        conv2d_full_oc_bias_kernel_dispatch::<T>([kh, kw], &mut 1, &mut ow_block);
    let bias_remain_oc_kernel = bn_remain_oc_kernel_dispatch::<T>([kh, kw], &mut ow_block);
    let bias_full_oc_ow_remain = conv2d_full_oc_bias_kernel_dispatch::<T>(
        [kh, kw],
        &mut oc_nvec,
        &mut ((out_width as usize) % ow_block),
    );
    let bias_one_oc_ow_remain = conv2d_full_oc_bias_kernel_dispatch::<T>(
        [kh, kw],
        &mut 1,
        &mut ((out_width as usize) % ow_block),
    );
    let bias_partial_oc_ow_remain =
        bn_remain_oc_kernel_dispatch::<T>([kh, kw], &mut ((out_width as usize) % ow_block));
    ...
    (0..outer).into_par_iter().for_each(|idx| {
        ...
        for ii in (0..in_channels).step_by(ic_block_size) {
            ...
            if i_end == in_channels {
                let bias = bias.map(|x| x.ptr());
...
fn with_normal_remain<T: CommonBounds>(
    ...
    inp: &Pointer<T>,
    activation: fn(T::Vec) -> T::Vec,
    full_oc_kernel: fn(Params, &mut Pointer<T>, &mut Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec),
