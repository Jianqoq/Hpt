...
impl<'a, T> ParallelIterator for ParStridedMut<'a, T> where T: CommonBounds {
    type Item = &'a mut T;
...
impl<'a, T> UnindexedProducer for ParStridedMut<'a, T> where T: CommonBounds {
    type Item = &'a mut T;

    fn split(self) -> (Self, Option<Self>) {
        let (a, b) = self.base.split();
        (
            ParStridedMut { base: a, phantom: std::marker::PhantomData },
            b.map(|x| ParStridedMut { base: x, phantom: std::marker::PhantomData }),
        )
    }
