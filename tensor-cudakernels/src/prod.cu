...
#define prod_bool(a, b) ((unsigned char)a) * ((unsigned char)b)
...
#define prod_f16(a, b) __half2float((a)) * __half2float((b))
...
#define DEFINE_REDUCE_KERNEL(rust_type, type)                                                                                                                                 \
    ...
    {                                                                                                                                                                         \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 32]));                                                \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 16]));                                                \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 8]));                                                 \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 4]));                                                 \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 2]));                                                 \
        sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + 1]));                                                 \
    }                                                                                                                                                                         \
    ...
    {                                                                                                                                                                         \
        ...
        {                                                                                                                                                                     \
            sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(in[i], in[i + blockDim.x]));                                                                      \
        }                                                                                                                                                                     \
        else if (i < size)                                                                                                                                                    \
        {                                                                                                                                                                     \
        ...
        {                                                                                                                                                                     \
            if (tid < s)                                                                                                                                                      \
            {                                                                                                                                                                 \
                sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[tid], sdata_##rust_type[tid + s]));                                         \
            }                                                                                                                                                                 \
            __syncthreads();                                                                                                                                                  \
        }                                                                                                                                                                     \
        if (tid < WRAP)                                                                                                                                                       \
    ...
    {                                                                                                                                                                         \
        ...
        {                                                                                                                                                                     \
            ...
            sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(in[a_offset], in[b_offset]));                                                                     \
        }                                                                                                                                                                     \
        else if (i < size)                                                                                                                                                    \
        {                                                                                                                                                                     \
            long long a_amount = i;                                                                                                                                           \
    ...
    {                                                                                                                                                                         \
        ...
        if (i + blockDim.x < cols)                                                                                                                                            \
        {                                                                                                                                                                     \
            sdata_##rust_type[tid] = cast_back_##rust_type(prod_##rust_type(in[i + blockIdx.y * cols], in[i + blockDim.x + blockIdx.y * cols]));                              \
        }                                                                                                                                                                     \
        else if (i < cols)                                                                                                                                                    \
        {                                                                                                                                                                     \
    ...
    {                                                                                                                                                                         \
        ...
        {                                                                                                                                                                     \
            ...
            {                                                                                                                                                                 \
                sdata_##rust_type[threadIdx.x] = cast_back_##rust_type(prod_##rust_type(sdata_##rust_type[threadIdx.x], sdata_##rust_type[s * blockDim.x + threadIdx.x]));    \
            }                                                                                                                                                                 \
