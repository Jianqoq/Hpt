...
#define atomicMin_bool(a, b)                           \
    ...
    (a) = min(((unsigned char)a), ((unsigned char)b)); \
    release_lock(&global_lock);

#define atomicMin_i8(a, b)      \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_u8(a, b)      \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_i16(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_u16(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_i64(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_u64(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_i32(a, b) atomicMin(&a, b)
#define atomicMin_u32(a, b) atomicMin(&a, b)

#define atomicMin_f32(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_f64(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min((a), (b));        \
    release_lock(&global_lock);

#define atomicMin_f16(a, b)     \
    acquire_lock(&global_lock); \
    (a) = min_f16(a, b);        \
    release_lock(&global_lock);
