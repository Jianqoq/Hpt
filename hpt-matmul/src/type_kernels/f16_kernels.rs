...
impl MatmulMicroKernel<f32, F32Vec> for half::f16 {
    type SelfVec = F16Vec;
    fn get_kernel(
        nr: usize,
        mr: usize,
    ...
    fn get_horizontal_kernel(
        ...
        assert_eq!(nr, 6);
        assert_eq!(mr, 1);
        define_matmul_micro_kernel!(half::f16, F16Vec, x6x1, 6, 1);
        x6x1
    }
    ...
    >(
        ...
        assert_eq!(nr, 6);
        assert_eq!(mr, 1);
        define_post_op_matmul_micro_kernel!(half::f16, F16Vec, x6x1, 6, 1);
        x6x1
    }
    ...
    fn get_mixed_precision_kernel(
        ...
        use crate::define_mixed_precision_matmul_micro_kernel;
        assert_eq!(nr, 1);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x1, 1, 1, 2);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x2, 1, 2, 2);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x3, 1, 3, 2);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x4, 1, 4, 2);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x5, 1, 5, 2);
        define_mixed_precision_matmul_micro_kernel!(half::f16, F16Vec, f32, F32Vec, x1x6, 1, 6, 2);
