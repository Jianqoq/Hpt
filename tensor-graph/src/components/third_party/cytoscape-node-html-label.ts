...
(function () {
  ...
  function updateDataOrStyleCyHandler(ev: ICyEventObject, params: CytoscapeNodeHtmlParams[], lc: LabelContainer) {
    setTimeout(() => {
      const target = ev.target;
      const param = $$find(params.slice().reverse(), x => target.is(x.query));
      if (param && !target.removed()) {
        lc.addOrUpdateElem(target.id(), param, {
          position: getNodePosition(target),
          data: target.data()
        });
      } else {
        lc.removeElemById(target.id());
      }
    }, 0);
  }
