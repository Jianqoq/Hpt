...
macro_rules! impl_matmul {
    ($tensor_type: ident, $func_name: ident, $func_name2: ident) => {
        /// Performs matrix multiplication without an output tensor.
        ///
        /// Given two tensors, this function multiplies them following the rules of matrix multiplication.
        /// It automatically handles broadcasting of shapes, if necessary.
        ///
        /// # Arguments
        /// - `lhs`: Reference to the left-hand side tensor.
        /// - `rhs`: Reference to the right-hand side tensor.
        ///
        /// # Returns
        /// A `Result` containing either the product tensor or an error.
        ///
        /// # Errors
        /// Returns an error if shapes are not compatible for matrix multiplication.
        pub(crate) fn $func_name<A, B>(
            lhs: &$tensor_type<A>,
            rhs: &$tensor_type<B>,
            ...
            <A as NormalOut<B>>::Output: CommonBounds,
        {
            if lhs.shape().len() == 2 && rhs.shape().len() == 2 {
                ...
                let (longer_shape, padded_short_shape) =
                    compare_and_pad_shapes(&lhs.shape(), &rhs.shape());
                let a_shape;
                let b_shape;
                ...
                if a_shape[a_shape.len() - 1] != b_shape[b_shape.len() - 2] {
                    ...
                    // a last dim == b second last dim && a_shape.len() == b_shape.len() && a_shape.len() > 2
                    // let's check if we can broadcast
        ...
        where
            ...
            B: CommonBounds + IntoScalar<<A as NormalOut<B>>::Output>,
            O: TensorLike<
                    <A as NormalOut<B>>::Output,
                    Output = $tensor_type<<A as NormalOut<B>>::Output>,
            ...
            <A as NormalOut<B>>::Output: CommonBounds,
        {
            if lhs.shape().len() == 2 && rhs.shape().len() == 2 {
                ...
                if lhs.shape()[1] != rhs.shape()[0] {
                    ...
                    let res;
                    if out.size() == ((lhs.shape()[0] * rhs.shape()[1]) as usize)
                        && out.parent().is_none()
                    {
                        let val = <A as NormalOut<B>>::Output::ZERO;
                        out.to_raw_mut().par_iter_mut().for_each(|x| {
                            *x = val;
                        });
                        res = out.static_cast()?;
                    } else {
...
impl_matmul!(_Tensor, matmul_no_out, matmul_with_out);
