                ...
                for m in 0..kernel_width {
                    for k in 0..out_width {
                        for j in 0..out_channels {
                            out[l][k][j] =
                                out[l][k][j] + inp[l * stride_height + n][k * stride_width + m][j];
                        }
                    }
                }
            }
    ...
    ($num:tt, [$($idx:expr),*]) => {
        paste::paste! {
            ...
            pub(crate) fn [<micro_kernel_ $num>]<T, const REGNUM: usize>(
                ...
                where T: CommonBounds, T::Vec: VecTrait<T> + Copy + Init<T> + VecCommon + NormalOut<Output = T::Vec>
            {
                $(
                    let _k = kp * (CONV_REGNUM as i64) + $idx;
                    let [<inp_vec $idx>] = unsafe { T::Vec::from_ptr(&inp[inp_offset + _k * step_width * isw] as *const _ as *const T) };
                )*
                $(
                    outs[$idx] = [<inp_vec $idx>]._add(outs[$idx]);
                )*
            }
        }
    };
    ...
    ($num:tt, [$($idx:expr),*]) => {
        paste::paste! {
            ...
            pub(crate) fn [<micro_kernel_ $num _with_buffer>]<T>(
                ...
                where T: CommonBounds, T::Vec: VecTrait<T> + Copy + Init<T> + NormalOut<Output = T::Vec>
            {
                ...
                for j in 0..num_co_rb + 1 {
                    unsafe {
                        $(
                            let _k = kp * (CONV_REGNUM as i64) + $idx;
                            let [<inp_vec $idx>] = T::Vec::from_ptr(&inp[inp_offset + _k * step_width * isw + j * (T::Vec::SIZE as i64)] as *const _ as *const T);
                        )*
                        #[cfg(not(feature = "bound_check"))]
                        ...
                        $(
                            let [<res $idx>] = [<inp_vec $idx>]._add([<out_vec $idx>].read_unaligned());
                        )*
                        $(
