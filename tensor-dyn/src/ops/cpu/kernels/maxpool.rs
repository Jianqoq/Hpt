...
use tensor_macros::{
    ...
    dwconv2d_microkernel_gen_pad_inps, maxpool2d_microkernel_gen_results,
};
...
pub struct ConvPartialKernel<T: CommonBounds> {
    pub(crate) kernel:
        fn(PartialParams, &mut Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec),
...
pub struct ConvKernel<T: CommonBounds> {
    pub(crate) kernel:
        fn(Params, &mut Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec),
}

impl<T: CommonBounds> ConvKernel<T> {
    pub(crate) fn new(
        kernel: fn(Params, &mut Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec),
    ) -> Self {
        Self { kernel }
    }
}
...
macro_rules! repeat_results {
    ($results:ident, $inp:ident, $i:ident, [$($idx:expr),*]) => {
        $(
            $results[$i as usize][$idx] = $inp[$idx]._max($results[$i as usize][$idx]);
        )*
    };
}
            ...
            if m_must_in_range {
                for m in 0..kw {
                    ...
                    for i in 0..IC_BLOCK as i64 {
                        ...
                        maxpool2d_microkernel_gen_results!(results, inp, kernel0, template_function);
                    }
                }
            } else {
                for m in 0..kw {
            ...
            if m_must_in_range {
                for m in 0..kw {
                    ...
                    for i in 0..OC_BLOCK as i64 {
                        ...
                        maxpool2d_microkernel_gen_results!(results, inp, none, template_function);
                    }
                }
            } else {
                for m in 0..kw {
...
where
    bool: IntoScalar<T>,
{
    let kernels: [[fn(Params, &mut Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec);
        5]; 4] = [
        [
            micro_kernel_1x1,
            micro_kernel_2x1,
...
pub(crate) fn conv2d_full_oc_bias_kernel_dispatch<T: CommonBounds>(
    ...
    fn(Params, &mut Pointer<T>, &Pointer<T>, &Pointer<T>, fn(T::Vec) -> T::Vec),
>
where
    bool: IntoScalar<T>,
{
    ...
    *kb = map_kb + 1;
    ...
    let kernel_fn = function1.get(map_ic).map(|x| x.get(map_kb)).flatten();
