...
use crate::ops::cpu::kernels::lp_pool_kernels::*;
...
fn case1_helper<T, const REGNUM: usize>(
    ...
    where T: CommonBounds + IntoScalar<T>, T::Vec: FloatOutBinary<Output = T::Vec>
{
...
fn case1_remain1_helper<T, const REGNUM: usize>(
    ...
    inp_cpy: &Pointer<T>,
    mut out: Pointer<T>,
    micro_kernel: fn(i64, i64, i64, i64, T, &Pointer<T>, &mut [T; REGNUM])
)
    where T: CommonBounds + IntoScalar<T> + NormalOut<Output = T> + FloatOutBinary<Output = T>
{
    ...
    for h in 0..REGNUM as i64 {
        ...
        *out_vec = res_buffer[h as usize]._pow(frac_1_p);
    }
}
    ...
    if remain == 0 {
        ...
        for j in 0..num_vec_size {
            ...
            for r in 0..REGNUM as i64 {
                buffers[r as usize] = buffers[r as usize]._pow(fract_1_p_vec);
            }
        }
        ...
        for r in 0..REGNUM {
            ...
            for i in 0..remain {
                buffer[i as usize] = buffer[i as usize]._pow(frac_1_p);
            }
        }
        store_fn(
            num_vec_size,
            ...
            FloatOutBinary<Output = T::Vec>
{
    ...
    ///
    /// This method performs Lp pooling, which computes the Lp norm over each local region defined by
    /// the `kernel_shape`. Lp pooling is a generalization of average pooling and max pooling, where `p`
    /// controls the pooling operation. Specifically, the elements in each region are raised to the power
    /// of `p`, summed, and then the p-th root of the sum is taken.
    ///
    /// # Arguments
    ///
    /// * `p` - The order of the norm (Lp norm) used for pooling. For example:
    ///   - `p = 1` corresponds to average pooling,
    ///   - `p = ∞` corresponds to max pooling,
    ///   - Other values of `p` generalize to Lp pooling.
    /// * `kernel_shape` - A 2-element array specifying the height and width of the pooling kernel (i.e., the local region).
    /// * `steps` - A 2-element array specifying the stride of the pooling operation along the height and width dimensions.
    /// * `padding` - A 2-element array of tuples specifying the padding added to the height and width dimensions. Each tuple contains the padding before and after the region along that dimension.
    /// * `dilation` - A 2-element array specifying the dilation factor for the pooling operation in the height and width dimensions.
    /// * `config` - An optional reference to a `Conv2dConfig` that holds configuration parameters for optimizing the pooling operation. If not provided, default settings are used.
    ///
    /// # Returns
    ///
    /// This function returns a `Result` containing a new tensor with the result of the Lp pooling operation.
        ...
        let case0_remain1 = move |b: i64, l: i64, c: i64, mut out: Pointer<T>| {
            for kp in 0..num_wo_b {
                ...
                for h in 0..CONV_REGNUM as i64 {
                    ...
                    *out_vec = res_buffer[h as usize]._pow(fract_1_p);
                }
            }
        };
        ...
        | {
            ...
            for kp in 0..num_wo_b {
                ...
                for r in 0..CONV_REGNUM {
                    ...
                    for i in 0..remain {
                        buffer[i as usize] = buffer[i as usize]._pow(fract_1_p);
                    }
                }
