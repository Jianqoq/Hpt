...
use super::kernels::conv::kernels::ConvKernel;
        ...
        T: ConvKernel,
    {
        ...
        (0..outer).into_par_iter().for_each(|idx| {
            ...
            for k in (0..out_width).step_by(OW_BLOCK) {
                ...
                for j in (0..out_channels).step_by(T::Vec::SIZE) {
                    let mut results = [T::Vec::splat(T::ZERO); OW_BLOCK];
                    for n in 0..kernel_height {
                        for m in 0..kernel_width {
                            let inp_offset =
                                b * isb + (l * step_height + n) * ish + (k * step_width + m) * isw;
                    ...
                    T::conv2d_nhwc_store(b * osb + l * osh + k * osw + j, osw, &results, &mut out);
                }
            }
        });
