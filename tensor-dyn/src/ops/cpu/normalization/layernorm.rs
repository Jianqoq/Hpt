...
impl<T, const DEVICE: usize> _Tensor<T, Cpu, DEVICE> {
        ...
        usize: Cast<<T as FloatOutBinary>::Output>,
    {
        ...
        match (gamma, beta) {
            (None, None) => Ok(res),
            (None, Some(beta)) => tensor_traits::NormalBinOps::add_(&res, beta, res.clone()),
            (Some(gamma), None) => tensor_traits::NormalBinOps::mul_(&res, gamma, res.clone()),
