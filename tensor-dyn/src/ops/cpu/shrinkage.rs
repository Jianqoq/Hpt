        ...
        <T::Vec as SimdCmp>::Output: SimdSelect<T::Vec>
{
    ...
    #[allow(unused)]
    pub fn shrinkage(&self, bias: T, lambda: T) -> anyhow::Result<_Tensor<T>> {
        ...
        <T::Vec as SimdCmp>::Output: SimdSelect<T::Vec>
{
    ...
    ///
    /// This function returns a `Result` containing a new tensor with the shrinkage operation applied.
    pub fn shrinkage(&self, bias: T, lambda: T) -> anyhow::Result<Tensor<T>> {
        Ok(_Tensor::<T>::shrinkage(self.inner.as_ref(), bias, lambda)?.into())
    }
}
