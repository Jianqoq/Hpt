...
impl<T> _Tensor<T>
    where
        ...
        bool: IntoScalar<T>, i64: IntoScalar<T>, 
        T::Vec: FloatOutBinary<T::Vec,Output = T::Vec>

    
{
 /// Performs a 2D average pooling operation on the input tensor.
///
/// This method applies a 2D average pooling operation on the tensor using the specified kernel,
/// strides (steps), padding, and dilation factors.
///
/// # Arguments
///
/// * `kernels_shape` - A reference to the shape representing the pooling kernel dimensions.
/// * `steps` - A 2-element array specifying the stride (step size) of the pooling along the height and width dimensions.
...
/// * `dilation` - A 2-element array specifying the dilation factor for the pooling along the height and width dimensions.
///   Dilation allows the kernel to be applied to inputs with gaps, increasing the receptive field of the kernel.
///
/// # Returns
///
/// This function returns a `Result` containing the output tensor after applying the 2D average pooling operation.
    ...
    pub fn avg_pool2d(
        ...
        //a new empty tensor for the output
        ...
        let kernel_size = (kernel_height * kernel_width).into_scalar();
        ...
        (0..out_size).into_par_iter().for_each(|idx| {
            ...
            for ii in (0..in_channels - in_channel_remain).step_by(IC_BLOCK_SIZE * T::Vec::SIZE) {
                ...
                //Logic to write to the register to memory
                //avg pooling will be calculated here
                for (idx, vec) in res_vecs.iter().enumerate() {
                    ...
                    unsafe {
                        ...
                        out_vec.write_unaligned(*vec._div(T::Vec::splat(kernel_size)) );
                    }
                }
            }
            ...
            for ii in (in_channels - in_channel_remain..in_channels - remain).step_by(
                ...
                let out_vec = (unsafe { out.ptr.add(out_idx as usize) }) as *mut T::Vec;
                unsafe {
                    out_vec.write_unaligned(*res_vecs._div(T::Vec::splat(kernel_size)));
                }
            }
            ...
            for ii in in_channels - remain..in_channels {
                ...
                unsafe {
                    out.write_unaligned(*res_vecs._div(T::Vec::splat(kernel_size)));
                }
            }
        });
