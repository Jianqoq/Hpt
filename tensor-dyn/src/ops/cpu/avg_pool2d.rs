...
{
    /// Applies a 2D average pooling operation on the tensor.
    ///
    /// This function performs a 2D average pooling operation, which is commonly
    /// used in downsampling layers of neural networks. The pooling operation
    /// reduces the spatial dimensions of the input tensor by averaging values
    /// within a sliding window (kernel) over the input. The output tensor has
    /// reduced size based on the kernel size, strides, padding, and dilation
    /// parameters.
    ///
    /// # Arguments
    ///
    /// * `kernel_shape` - A 2-element array representing the size of the pooling
    ///   window (kernel) along the two spatial dimensions [height, width].
    /// * `steps` - A 2-element array specifying the stride of the pooling window
    ///   in the height and width dimensions, i.e., how far the window moves between
    ///   applications of pooling.
    /// * `padding` - A 2-element array of tuples, each tuple representing the padding
    ///   values (before and after) for the height and width dimensions.
    /// * `dilation` - A 2-element array specifying the dilation factor for the
    ///   pooling window in the height and width dimensions.
    /// * `config` - An optional reference to a `Conv2dConfig` struct that holds
    ///   additional configuration parameters such as bias, groups, and others
    ///   related to the 2D convolution/pooling operation.
    ///
    /// # Returns
    ///
    /// This function returns a result containing the output tensor after applying
    /// the average pooling operation. In case of errors, it returns an
    /// `anyhow::Result` with an appropriate error message.
