...
pub fn common_gelu(
    ...
    let func = |stage_out_id: usize| {
        ...
        var.clone() *
            dtype_point5(res_dtype) *
            (dtype_one(res_dtype) + erf(var.clone() / dtype_sqrt2(res_dtype)))
    };
    if is_output {
        common_gelu_helper(
            inputs,
        ...
        common_gelu_helper(inputs, shape, output_id, res_dtype, |_, stage_out_id: usize, _| {
            Body::Stmt(
                Stmt::LetStmt(
                    LetStmt::make(
        ...
        *self.id.borrow_mut() += 1;
        ...
        let ret = Tensor {
            ...
            body_gen: Arc::new(move |inputs: Vec<Body>, is_output: bool, id: usize| {
                common_gelu(is_output, &inputs, &shape, res_dtype, id)
            }),
        };
