...
use hpt_core::{Tensor, TensorCreator, Conv};
...
fn assert_eq(
    ...
    let res = a
        .dwconv2d(&a_kernel, None, [1, 1], [(0, 0), (0, 0)], [1, 1], None)?
        .permute([0, 3, 1, 2])?
        .contiguous()?;
    let res2 = b.conv2d(
        &b_kernel,
...
fn assert_eq_pad(
    ...
    let res = a
        .dwconv2d(&a_kernel, None, [1, 1], [(2, 2), (2, 2)], [1, 1], None)?
        .permute([0, 3, 1, 2])?
        .contiguous()?;
    let res2 = b.conv2d(
        &b_kernel,
