...
fn assert_eq(hpt_res: &Tensor<TestTypes>, tch_res: &tch::Tensor) -> anyhow::Result<()> {
    let tch_res = unsafe {
        Tensor::<TestTypes>::from_raw(tch_res.data_ptr() as *mut TestTypes, &hpt_res.shape().to_vec())
    }?;
...
fn test_hann() -> anyhow::Result<()> {
    let tch_a =
        tch::Tensor::hann_window_periodic(1000, true, (TCH_TEST_TYPES, tch::Device::Cpu));
...
fn test_tril() -> anyhow::Result<()> {
    fn assert(diagnal: i64) -> anyhow::Result<()> {
        let tch_a =
            tch::Tensor::randn(&[10, 10], (TCH_TEST_TYPES, tch::Device::Cpu)).tril(diagnal);
