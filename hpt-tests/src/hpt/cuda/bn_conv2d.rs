...
fn assert_eq(
    ...
    let a = a.to_cuda::<0>()?;
    let oc = *a_kernel.shape().first().unwrap();
    let mean =
    ...
    let beta =
        tch::Tensor::arange(oc, (tch::Kind::Float, tch::Device::Cpu)).reshape(&[1, oc, 1, 1]);
    let hpt_mean = Tensor::<f32, Cuda>::arange(0, oc)?;
    let hpt_var = Tensor::<f32, Cuda>::arange(0, oc)?;
    let hpt_gamma = Tensor::<f32, Cuda>::arange(0, oc)?;
    let hpt_beta = Tensor::<f32, Cuda>::arange(0, oc)?;
    let res = a
        .batchnorm_conv2d(
            &a_kernel,
            &hpt_mean,
...
fn assert_eq_bias(
    ...
    let hpt_gamma = Tensor::<f32, Cuda>::arange(0, oc)?;
    let hpt_beta = Tensor::<f32, Cuda>::arange(0, oc)?;
    let bias = Tensor::<f32, Cuda>::arange(0i64, oc)?;
    let res = a
        .batchnorm_conv2d(
            &a_kernel,
            &hpt_mean,
