...
fn test_case0() -> anyhow::Result<()> {
    set_num_threads(1);
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 1, 1, 5, 5])?;
    ...
    // assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    // assert_eq_bias_pad_relu6(&a, &kernel, &tch_a, &tch_kernel)?;
    // let (kernel, a, tch_kernel, tch_a) = common_input([1, 2, 3, 5, 5])?;
    // assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    // assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    // assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    // assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
...
fn test_case1() -> anyhow::Result<()> {
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 32, 3, 20, 20])?;
    assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad_relu6(&a, &kernel, &tch_a, &tch_kernel)?;
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 32, 3, 5, 5])?;
    assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
...
fn test_case5() -> anyhow::Result<()> {
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 32, 3, 19, 19])?;
    assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
...
fn test_case6() -> anyhow::Result<()> {
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 30, 3, 20, 20])?;
    assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad_relu6(&a, &kernel, &tch_a, &tch_kernel)?;
    let (kernel, a, tch_kernel, tch_a) = common_input([1, 30, 3, 5, 5])?;
    assert_eq(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_pad(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias(&a, &kernel, &tch_a, &tch_kernel)?;
    assert_eq_bias_pad(&a, &kernel, &tch_a, &tch_kernel)?;
