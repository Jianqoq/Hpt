...
impl SimdCompare for f16x16 {
    ...
    fn simd_eq(self, other: Self) -> i16x16 {
        let x: Simd<i16, 16> = unsafe { std::mem::transmute(self.0) };
        let y: Simd<i16, 16> = unsafe { std::mem::transmute(other.0) };
        let eq = x.simd_eq(y);
...
pub fn u16_to_f16(val: u16x8) -> std::simd::f32x8 {
    ...
    let infinity_or_nan_mask = half_exp.simd_eq(std::simd::u32x8::splat(0x7c00));
    let nan_mask = half_man.simd_ne(std::simd::u32x8::splat(0));
    let infinity_mask = half_man.simd_eq(std::simd::u32x8::splat(0));

    let inf_result = (half_sign << 16) | std::simd::u32x8::splat(0x7f80_0000);
    let nan_result = (half_sign << 16) | std::simd::u32x8::splat(0x7fc0_0000) | (half_man << 13);
    ...
    let subnormal_mask = half_exp.simd_eq(std::simd::u32x8::splat(0));
