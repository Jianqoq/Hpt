...
use arch_simd::sleef::arch::helper::vfma_vf_vf_vf_vf;

use crate::*;

#[inline(always)]
pub(crate) unsafe fn visnegzero_vo_vf(d: VFloat) -> Vopmask {
    veq_vo_vi2_vi2(vreinterpret_vi2_vf(d), vreinterpret_vi2_vf(vcast_vf_f(-0.0)))
}
...
pub(crate) unsafe fn rempisubf(x: VFloat) -> Fit {
    ...
    let rint4x = vsel_vf_vo_vf_vf(
        vgt_vo_vf_vf(vabs_vf_vf(vmul_vf_vf_vf(vcast_vf_f(4.0), x)), vcast_vf_f((1 << 23) as f32)),
...
pub(crate) unsafe fn rempif(a: VFloat) -> Dfit {
    ...
    x = dfmul_vf2_vf2_vf2(
        x,
        vcast_vf2_f_f(3.1415927410125732422f32 * 2.0, -8.7422776573475857731e-8f32 * 2.0)
    );
...
pub(crate) unsafe fn xtanf_u1(d: VFloat) -> VFloat {
    ...
    u = vmla_vf_vf_vf_vf(u, vf2getx_vf_vf2(s), vcast_vf_f(-8.3920182078145444393158e-5f32));
    ...
    x = dfadd_vf2_vf_vf2(
        vcast_vf_f(1.0),
        dfmul_vf2_vf2_vf2(
            dfadd_vf2_vf_vf2(vcast_vf_f(0.33333361148834228515625f32), dfmul_vf2_vf2_vf2(s, x)),
            s
        )
    );
...
pub(crate) unsafe fn xasinf_u1(d: VFloat) -> VFloat {
    ...
    let x2 = vsel_vf_vo_vf_vf(
        o,
        vmul_vf_vf_vf(d, d),
        vmul_vf_vf_vf(vsub_vf_vf_vf(vcast_vf_f(1.0), vabs_vf_vf(d)), vcast_vf_f(0.5f32))
    );
    ...
    let y = dfsub_vf2_vf2_vf(
        dfsub_vf2_vf2_vf2(
            vcast_vf2_f_f(3.1415927410125732422f32 / 4.0, -8.7422776573475857731e-8f32 / 4.0),
            x
        ),
        u
    );
    ...
    let r = vsel_vf_vo_vf_vf(
        ...
        vmul_vf_vf_vf(vadd_vf_vf_vf(vf2getx_vf_vf2(y), vf2gety_vf_vf2(y)), vcast_vf_f(2.0))
    );
...
pub(crate) unsafe fn xacosf_u1(d: VFloat) -> VFloat {
    ...
    y = vsel_vf2_vo_vf2_vf2(
        ...
        dfsub_vf2_vf2_vf2(vcast_vf2_f_f(3.1415927410125732422f32, -8.7422776573475857731e-8f32), y),
        y
    );
...
pub(crate) unsafe fn xatanf_u1(d: VFloat) -> VFloat {
    let d2 = atan2kf_u1(vcast_vf2_vf_vf(vabs_vf_vf(d), vcast_vf_f(0.0)), vcast_vf2_f_f(1.0, 0.0));
...
pub(crate) unsafe fn xlogf_u1(d: VFloat) -> VFloat {
    ...
    #[cfg(not(target_feature = "avx512f"))]
    {
        ...
        r = vsel_vf_vo_vf_vf(veq_vo_vf_vf(d, vcast_vf_f(0.0)), vcast_vf_f(f32::NEG_INFINITY), r);
    }
...
pub(crate) unsafe fn xcbrtf_u1(d: VFloat) -> VFloat {
    ...
    re = vtruncate_vi2_vf(vsub_vf_vf_vf(t, vmul_vf_vf_vf(vcast_vf_vi2(qu), vcast_vf_f(3.0))));
    ...
    y = vmul_vf_vf_vf(x, x);
    y = vmul_vf_vf_vf(y, y);
    x = vsub_vf_vf_vf(x, vmul_vf_vf_vf(vmlanp_vf_vf_vf_vf(d, y, x), vcast_vf_f(-1.0 / 3.0)));
...
pub(crate) unsafe fn xexpf(d: VFloat) -> VFloat {
    ...
    u = vreinterpret_vf_vm(
        vandnot_vm_vo32_vm(vlt_vo_vf_vf(d, vcast_vf_f(-104.0)), vreinterpret_vm_vf(u))
    );
    u = vsel_vf_vo_vf_vf(vlt_vo_vf_vf(vcast_vf_f(100.0), d), vcast_vf_f(f32::INFINITY), u);
...
unsafe fn expkf(d: VFloat2) -> VFloat {
    let u = vmul_vf_vf_vf(vadd_vf_vf_vf(vf2getx_vf_vf2(d), vf2gety_vf_vf2(d)), vcast_vf_f(R_LN2_F));
...
unsafe fn logkf(d: VFloat) -> VFloat2 {
    ...
    let c = vcast_vf2_f_f(0.66666662693023681640625f32, 3.69183861259614332084311e-9f32);
    ...
    s = dfadd_vf2_vf2_vf2(
        s,
        dfmul_vf2_vf2_vf2(dfmul_vf2_vf2_vf2(x2, x), dfadd2_vf2_vf2_vf2(dfmul_vf2_vf2_vf(x2, t), c))
    );
...
pub(crate) unsafe fn xpowf(x: VFloat, y: VFloat) -> VFloat {
    ...
    let yisodd = vand_vo_vo_vo(
        vand_vo_vo_vo(
            veq_vo_vi2_vi2(vand_vi2_vi2_vi2(vtruncate_vi2_vf(y), vcast_vi2_i(1)), vcast_vi2_i(1)),
            yisint
        ),
    ...
    result = vreinterpret_vf_vm(
        vor_vm_vo32_vm(vor_vo_vo_vo(visnan_vo_vf(x), visnan_vo_vf(y)), vreinterpret_vm_vf(result))
    );

    result = vsel_vf_vo_vf_vf(
        vor_vo_vo_vo(veq_vo_vf_vf(y, vcast_vf_f(0.0)), veq_vo_vf_vf(x, vcast_vf_f(1.0))),
        vcast_vf_f(1.0),
        result
    );
...
pub(crate) unsafe fn xsinhf(x: VFloat) -> VFloat {
    ...
    let mut y = vmul_vf_vf_vf(vadd_vf_vf_vf(vf2getx_vf_vf2(d), vf2gety_vf_vf2(d)), vcast_vf_f(0.5));

    y = vsel_vf_vo_vf_vf(
        vor_vo_vo_vo(vgt_vo_vf_vf(vabs_vf_vf(x), vcast_vf_f(89.0)), visnan_vo_vf(y)),
        vcast_vf_f(f32::INFINITY),
        y
    );
...
pub(crate) unsafe fn xtanhf(x: VFloat) -> VFloat {
    ...
    y = vsel_vf_vo_vf_vf(
        vor_vo_vo_vo(vgt_vo_vf_vf(vabs_vf_vf(x), vcast_vf_f(8.664339742f32)), visnan_vo_vf(y)),
        vcast_vf_f(1.0),
        y
    );
    y = vmulsign_vf_vf_vf(y, x);
...
unsafe fn logk2f(d: VFloat2) -> VFloat2 {
    ...
    #[cfg(target_feature = "avx512f")]
    let e = vrint_vi2_vf(
        vgetexp_vf_vf(vmul_vf_vf_vf(vf2getx_vf_vf2(d), vcast_vf_f(1.0f32 / 0.75f32)))
    );
...
unsafe fn expk2f(d: VFloat2) -> VFloat2 {
    ...
    t = dfadd2_vf2_vf2_vf(dfmul_vf2_vf2_vf(s, u), vcast_vf_f(0.166666659414234244790680580464f32));
...
pub(crate) unsafe fn xatanhf(x: VFloat) -> VFloat {
    ...
    y = vreinterpret_vf_vm(
        vor_vm_vo32_vm(vor_vo_vo_vo(visinf_vo_vf(x), visnan_vo_vf(y)), vreinterpret_vm_vf(y))
    );
    y = vmulsign_vf_vf_vf(y, x);
    y = vreinterpret_vf_vm(vor_vm_vo32_vm(visnan_vo_vf(x), vreinterpret_vm_vf(y)));
...
pub(crate) unsafe fn xacoshf(x: VFloat) -> VFloat {
    ...
    y = vreinterpret_vf_vm(
        vandnot_vm_vo32_vm(veq_vo_vf_vf(x, vcast_vf_f(1.0)), vreinterpret_vm_vf(y))
    );

    y = vreinterpret_vf_vm(vor_vm_vo32_vm(vlt_vo_vf_vf(x, vcast_vf_f(1.0)), vreinterpret_vm_vf(y)));
    y = vreinterpret_vf_vm(vor_vm_vo32_vm(visnan_vo_vf(x), vreinterpret_vm_vf(y)));
...
pub(crate) unsafe fn xexp2f(d: VFloat) -> VFloat {
    ...
    {
        u = vf2getx_vf_vf2(
            dfnormalize_vf2_vf2(dfadd_vf2_vf_vf2(vcast_vf_f(1.0), dfmul_vf2_vf_vf(u, s)))
        );
    }

    u = vldexp2_vf_vf_vi2(u, q);

    u = vsel_vf_vo_vf_vf(vge_vo_vf_vf(d, vcast_vf_f(128.0)), vcast_vf_f(f32::INFINITY), u);
    u = vreinterpret_vf_vm(
        vandnot_vm_vo32_vm(vlt_vo_vf_vf(d, vcast_vf_f(-150.0)), vreinterpret_vm_vf(u))
    );
...
pub(crate) unsafe fn xexp10f(d: VFloat) -> VFloat {
    ...
    u = vf2getx_vf_vf2(
        dfnormalize_vf2_vf2(dfadd_vf2_vf_vf2(vcast_vf_f(1.0), dfmul_vf2_vf2_vf(x, s)))
    );
    ...
    u = vreinterpret_vf_vm(
        vandnot_vm_vo32_vm(vlt_vo_vf_vf(d, vcast_vf_f(-50.0)), vreinterpret_vm_vf(u))
    );
...
pub(crate) unsafe fn xexpm1f(a: VFloat) -> VFloat {
    let d = dfadd2_vf2_vf2_vf(expk2f(vcast_vf2_vf_vf(a, vcast_vf_f(0.0))), vcast_vf_f(-1.0));
...
pub(crate) unsafe fn xlog10f(d: VFloat) -> VFloat {
    ...
    #[cfg(not(target_feature = "avx512f"))]
    let mut s = dfmul_vf2_vf2_vf(vcast_vf2_f_f(0.30103001f32, -1.432098889e-8f32), vcast_vf_vi2(e));
...
pub(crate) unsafe fn xlog2f(d: VFloat) -> VFloat {
    ...
    let mut s = dfadd2_vf2_vf_vf2(
        vcast_vf_vi2(e),
        dfmul_vf2_vf2_vf2(x, vcast_vf2_f_f(2.8853900432586669922f32, 3.2734474483568488616e-8f32))
    );
...
pub(crate) unsafe fn xlog1pf(d: VFloat) -> VFloat {
    ...
    x = dfdiv_vf2_vf2_vf2(vcast_vf2_vf_vf(m, vcast_vf_f(0.0)), dfadd_vf2_vf_vf(vcast_vf_f(2.0), m));
    x2 = vmul_vf_vf_vf(vf2getx_vf_vf2(x), vf2getx_vf_vf2(x));
    ...
    r = vreinterpret_vf_vm(
        vor_vm_vo32_vm(vgt_vo_vf_vf(vcast_vf_f(-1.0), d), vreinterpret_vm_vf(r))
    );
    r = vsel_vf_vo_vf_vf(veq_vo_vf_vf(d, vcast_vf_f(-1.0)), vcast_vf_f(f32::NEG_INFINITY), r);
...
pub(crate) unsafe fn xsqrtf_u05(d: VFloat) -> VFloat {
    #[cfg(target_feature = "fma")]
    {
        ...
        y = vreinterpret_vf_vi2(
            vsub_vi2_vi2_vi2(vcast_vi2_i(0x5f3759df), vsrl_vi2_vi2_i::<1>(vreinterpret_vi2_vf(d)))
        );
    ...
    #[cfg(not(target_feature = "fma"))]
    {
        ...
        let d = vsel_vf_vo_vf_vf(o, vmul_vf_vf_vf(d, vcast_vf_f(1.888946593147858e22f32)), d);
        q = vsel_vf_vo_vf_vf(o, vcast_vf_f(7.275957614183426e-12f32 * 0.5), vcast_vf_f(0.5));
...
pub(crate) unsafe fn xroundf(d: VFloat) -> VFloat {
    ...
    let mut fr = vsub_vf_vf_vf(x, vcast_vf_vi2(vtruncate_vi2_vf(x)));
    x = vsel_vf_vo_vf_vf(
        vand_vo_vo_vo(vle_vo_vf_vf(x, vcast_vf_f(0.0)), veq_vo_vf_vf(fr, vcast_vf_f(0.0))),
    ...
    x = vsel_vf_vo_vf_vf(veq_vo_vf_vf(d, vcast_vf_f(0.4999999701976776123)), vcast_vf_f(0.0), x);
    vsel_vf_vo_vf_vf(
        vor_vo_vo_vo(
            visinf_vo_vf(d),
...
unsafe fn poly4df(x: VFloat, c3: VFloat, c2: VFloat2, c1: VFloat2, c0: VFloat2) -> VFloat2 {
    dfmla_vf2_vf_vf2_vf2(vmul_vf_vf_vf(x, x), poly2df(x, c3, c2), poly2df_b(x, c1, c0))
}
...
pub(crate) unsafe fn xerff_u1(a: VFloat) -> VFloat {
    ...
    t2 = vsel_vf2_vo_vf2_vf2(
        ...
        dfmul_vf2_vf2_vf(vcast_vf2_f_f(-1.1283792257308959961, 5.8635383422197591097e-8), x),
        t2
    );
