...
pub(crate) unsafe fn vtruncate2_vd_vd(x: VDouble) -> VDouble {
    let mut fr = vsub_vd_vd_vd(
        ...
        vmul_vd_vd_vd(
            ...
            vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, vcast_vd_d(1.0 / ((1i64 << 31) as f64)))))
        )
    );
    ...
    vsel_vd_vo_vd_vd(
        vor_vo_vo_vo(visinf_vo_vd(x), vge_vo_vd_vd(vabs_vd_vd(x), vcast_vd_d((1i64 << 52) as f64))),
        x,
...
pub(crate) unsafe fn vround2_vd_vd(d: VDouble) -> VDouble {
    ...
    x = vsel_vd_vo_vd_vd(
        vand_vo_vo_vo(vle_vo_vd_vd(x, vcast_vd_d(0.0)), veq_vo_vd_vd(fr, vcast_vd_d(0.0))),
    ...
    x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, vcast_vd_d(0.49999999999999994449)), vcast_vd_d(0.0), x);

    vsel_vd_vo_vd_vd(
        vor_vo_vo_vo(visinf_vo_vd(d), vge_vo_vd_vd(vabs_vd_vd(d), vcast_vd_d((1i64 << 52) as f64))),
        d,
...
pub(crate) unsafe fn vilogbk_vi_vd(d: VDouble) -> VInt {
    ...
    q = vsub_vi_vi_vi(
        q,
        vsel_vi_vo_vi_vi(vcast_vo32_vo64(o), vcast_vi_i(300 + 0x3ff), vcast_vi_i(0x3ff))
    );
    q
}
