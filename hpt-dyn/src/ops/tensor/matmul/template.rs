    ...
    F2: Fn(T::Vec, usize, usize) -> T::Vec + Clone + Send + Sync + 'static,
{
    ...
    let func = move |((((tid, prg), rem_prg), (start, end)), (start_rem, end_rem)): (
        (((usize, [usize; 3]), [usize; 3]), (usize, usize)),
        (usize, usize),
    )| {
    ...
    T: CommonBounds,
{
    ...
    for j in (start..nc).step_by(nr) {
        ...
        if nb == nr && stride == 1 {
            for p in 0..kb as i64 {
                unsafe {
                    let b_ptr = b.offset(((p * ldb) as isize) + (j as isize)).cast::<T::Vec>();
