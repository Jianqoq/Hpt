...
// use hpt_types::{ dtype::{ DType, TypeCommon }, type_promote::FloatOutUnary };
...
//         lens.clone()
//     } else {
//         Tensor::full(&[1], seq_length as f64, DType::I64, x.device.clone())?.expand(&[batch_size])?
//     };
...
//         // 批量计算X*W，优化性能
//         let xw = x_reshaped.matmul(&w_dir.t()?)?;
//         let xw = xw.reshape(&[seq_length, batch_size, 4 * hidden_size])?;

//         let xw_bias = if let Some(wb) = &w_bias { &xw + &wb } else { xw };
...
//             // 计算h_{t-1}*R
//             let ht_r = h_prev.matmul(&r_dir.t()?)?;

//             // 应用循环偏置
//             let ht_r_bias = if let Some(rb) = &r_bias { &ht_r + &rb } else { ht_r };
//             // 合并门控输入
...
//             let i_peep = if let Some(p) = &peep_weights {
...
//             let f_peep = if let Some(p) = &peep_weights {
