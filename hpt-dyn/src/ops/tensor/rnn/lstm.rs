    ...
    match p {
        ...
        None => {
            ...
            for h in 0..num_vec {
                ...
                y_vec_ptr
                    .offset(h as isize)
                    .write_unaligned(o_res._mul(tmp._tanh()));
    ...
    T::Vec: FloatOutUnary<Output = T::Vec>,
{
    ...
    if batch_parallel {
        ...
        chunks.into_par_iter().for_each(|(start, end)| {
            ...
            func(start as i64, end as i64, y_ptr, &y_layout, &prepacked_r)
    ...
    T::Vec: FloatOutUnary<Output = T::Vec>,
{
    ...
    if batch_parallel {
        let func =
            |start: i64, end: i64, y: Pointer<T>, y_layout: &Layout| -> Result<(), TensorError> {
                let gates =
                    Tensor::empty(&[end - start, 4 * hidden_size], x.dtype, x.device.clone())?;
                let mut total_post_time = std::time::Duration::ZERO;
                let h_t = initial_h.slice(&select![start:end, ..])?;
                let c_t = initial_c.slice(&select![start:end, ..])?;
...
where
    T: FloatOutUnary<Output = T> + ToDType + CommonBounds + hpt_matmul::MatmulMicroKernel + Zero,
    T::Vec: FloatOutUnary<Output = T::Vec>,
{
...
impl Tensor {
    ...
    pub(crate) fn lstm_onnx(
        ...
        let (y, y_h, y_c) = match self.dtype {
            ...
            #[cfg(feature = "f16")]
            DType::F16 => lstm_cell_onnx::<half::f16>(
                self,
                w,
                r,
                summed_bias,
