...
use hpt_types::{REGNUM, dtype::ToDType};
use hpt_types::{into_scalar::Cast, traits::VecTrait};

use crate::{Device, Tensor};

#[track_caller]
pub(crate) fn pooling_template<
    T: CommonBounds + ToDType,
    O: CommonBounds + ToDType,
>(
    img: &Tensor,
    kernels_shape: &Shape,
    steps: [i64; 2],
...
#[track_caller]
pub(crate) fn adaptive_pooling_template<T: CommonBounds + ToDType, O: CommonBounds + ToDType>(
    img: &Tensor,
    output_size: [i64; 2],
    scalar_op: impl Fn(T, T) -> T + Send + Sync,
    vec_op: impl Fn(T::Vec, T::Vec) -> T::Vec + Send + Sync,
