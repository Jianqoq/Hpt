...
use crate::{
    Tensor,
    ops::tensor::reduce::reduce_utils::{is_keep_fast_dim, rearrange_array},
};
...
pub(crate) fn normalize_prepare<T: CommonBounds>(
    a: &Tensor,
    res_dtype: DType,
    axis: usize,
    c: Option<Tensor>,
) -> std::result::Result<(bool, Tensor, Tensor), TensorError> {
    ...
    let res = if let Some(out) = c {
        ...
        Tensor::empty(a.shape(), res_dtype, a.device.clone())
    };
...
#[track_caller]
pub(crate) fn contiguous_normalize_template<T, O, F1, F2, F3>(
    a: &Tensor,
    axis: usize,
    res_dtype: DType,
    c: Option<Tensor>,
...
where
    T: CommonBounds,
    F1: Fn(&mut O),
    F2: Fn(usize, usize, &Tensor, &Tensor),
    F3: Fn(usize, usize, usize, &Tensor, &Tensor),
{
    let (keep_fast_dim, transposed_tensor, result) = normalize_prepare::<T>(a, res_dtype, axis, c)?;
    ...
    if a.ndim() == 1 {
        full_reduce(unsafe { (result.data.ptr as *mut O).as_mut().unwrap() });
    } else {
        if !keep_fast_dim {
...
#[track_caller]
pub(crate) fn uncontiguous_normalize_template<T, O, F1, F2, F3>(
    a: &Tensor,
    axis: usize,
    res_dtype: DType,
    c: Option<Tensor>,
