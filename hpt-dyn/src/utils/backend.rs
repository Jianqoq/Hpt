...
impl Backend {
    ...
    pub fn dealloc(&self, layout: std::alloc::Layout) {
        match self {
            Self::Cpu { should_drop, ptr, device_id } => {
                use hpt_allocator::Cpu;
                let allocator = hpt_allocator::HptAllocator::<Cpu>::new();
                allocator.deallocate(ptr.ptr, &layout, *should_drop, *device_id);
            }
            #[cfg(feature = "cuda")]
            Self::Cuda { should_drop, ptr, device, .. } => {
                if *should_drop {
...
impl Clone for Backend {
    fn clone(&self) -> Self {
        match self {
            Self::Cpu { should_drop, ptr, device_id } => {
                clone_storage(ptr.ptr, *device_id, &*hpt_allocator::CPU_STORAGE);
                Self::Cpu {
            ...
            #[cfg(feature = "cuda")]
            Self::Cuda { should_drop, ptr, device, cap } => {
...
impl std::fmt::Debug for Backend {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Backend::Cpu { should_drop, ptr, device_id } =>
                f
                    ...
                    .finish(),
            #[cfg(feature = "cuda")]
            Backend::Cuda { should_drop, ptr, device, cap } =>
                f
