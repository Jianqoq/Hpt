use hpt_common::{error::base::TensorError, utils::conv_algos::ConvAlgo};
...
/// trait for conv operations with cuda backend
pub trait CudaConv<T: CommonBounds> {
    /// the output type of the conv operation
    type Output;
...
/// trait for conv operations with batch normalization on cuda
pub trait CudaConvBatchNorm<T: CommonBounds> {
    /// the output type of the conv operation
    type Output;
    ...
    /// ```rust
    /// let input = Tensor::<f32, Cuda>::randn([1, 32, 32, 3])?;
    /// let kernels = Tensor::<f32, Cuda>::randn([3, 3, 3, 16])?;
