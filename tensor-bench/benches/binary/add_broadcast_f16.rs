...
fn add_f16_benchmark(c: &mut Criterion) {
    ...
    for idx in 0..shapes.len() {
        ...
        shape2[1] = 1;
        shape2[3] = 1;
        let a = black_box(Tensor::randn(shape1, (Kind::Half, Device::Cpu)));
        let c = black_box(Tensor::randn(shape2, (Kind::Half, Device::Cpu)));
        let a2 = black_box(_Tensor::<f16>::randn(shape1).unwrap());
        let c2 = black_box(_Tensor::<f16>::randn(shape2).unwrap());
